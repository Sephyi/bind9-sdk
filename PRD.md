...
| --- | --- | --- |
| 1.8 | 2026-06-20 | Supply-chain and fuzzing milestones (CX-17, CX-05). Added `deny.toml` (advisories/bans/licenses/sources) gating CI with `cargo deny check` green; a `1.94.0`/`stable` MSRV matrix; a `release.yml` workflow performing the supply-chain gate, dependency-ordered `cargo package` publish dry run, CycloneDX SBOM with SHA-256 checksums, and build-provenance attestation; a hardened `[profile.release]` plus dedicated `[profile.fuzz]`; and a version on the CLI's `bind9-sdk` path dependency for crates.io publishability. Implemented FR-061: a detached `fuzz/` cargo-fuzz workspace with eight targets (zone, rndc response, stats JSON, named.conf, zone roundtrip, TSIG wire, transfer wire, update response), feature-gated `fuzz` entry modules, seed corpora, deterministic `--features fuzzing` regression tests, and a bounded CI smoke-fuzz job. Fuzzing surfaced and fixed a real stack-overflow DoS: the zone tokenizer recursed per parenthesized newline / comment line; it now scans iteratively. Added PRD SR-005 input-size (256 MiB) and record-count (16M) guards to the zone parser. The 24-hour fuzz run remains an explicit pre-v1.0 release gate (not yet performed). Verification: 696 deterministic workspace tests pass (388 core, 285 net, 21 CLI, one trybuild, one doctest), all-feature and production-panic clippy gates pass, both no-default-feature WASM checks pass, and `cargo deny check` is clean. |
| 1.7 | 2026-06-20 | Promoted the live BIND 9.20 protocol suite into a mandatory GitHub Actions job using the repository fixture through Docker Compose, with health polling, all ignored/live tests, diagnostic logs on startup failure, and unconditional teardown. CI and local `mise` clippy gates now use all features and separately deny production `unwrap`, `expect`, `panic`, and `unreachable` paths. |
| 1.6 | 2026-06-20 | Completed FR-060 with a bounded `no_std + alloc` `named.conf` parser. Typed models cover primary/secondary/stub/forward zones, file paths, transfer ACLs, TSIG key blocks, named ACLs, recursion policy, RNDC controls, and statistics-channel listeners. Address-match elements use typed IP/CIDR/domain variants; key material is returned as redacting, zeroizing `TsigKey` values. Unknown BIND directives are skipped structurally instead of pretending to implement the full grammar, while malformed supported directives report source path and line. The parser accepts the repository's live BIND 9.20 fixture. OQ-003 is resolved to this explicit scope. Verification: 689 deterministic workspace tests pass (381 core, 285 net, 21 CLI, one trybuild, one doctest), all-feature and production panic clippy gates pass, and core/top-level no-default-feature WASM checks pass. |
| 1.5 | 2026-06-20 | TDD hardening closed five additional combined-audit findings. AXFR and IXFR reject records after protocol completion in the same DNS message. Transfer wire-name parsing preserves arbitrary DNS label octets through master-file escapes instead of requiring UTF-8. High-level `Bind9Client`, `RndcPool`, and CLI configuration now retain loopback-only RNDC by default while providing an explicit `ProtectedNetwork`/`--protected-rndc` opt-in for WireGuard or equivalent trusted overlays. Production panic paths were removed from DNS labels, zone serial lookup, transfer TTL fallback, CLI runtime/output/zone rendering, static UDP binding, and TSIG HMAC construction. `TsigKey::sign` and `TsigRecord::new` are now fallible; TSIG construction also rejects timestamps outside its 48-bit wire field and oversized prior MACs. `NamedControl` now exposes `thaw`, and `Bind9Client::freeze_guard` returns a `FrozenZoneGuard` with explicit awaited thaw plus best-effort panic/drop cleanup on its captured Tokio runtime. Verification: 684 deterministic workspace tests pass (376 core, 285 net, 21 CLI, one trybuild, one doctest), production-target panic linting and all-feature clippy are warning-free, both core and top-level `wasm32-unknown-unknown` no-default-feature checks pass, and all 28 live BIND 9.20 tests pass. |
| 1.4 | 2026-06-20 | Independent and fresh production-readiness audits consolidated in `docs/audit-combined-2026-06-20.md`. TDD remediation closed transfer/update fail-open paths: AXFR now rejects mismatched opening/closing SOA serials; IXFR rejects RFC 1982 undefined half-space ordering; transfer IDs, rndc nonces, and update IDs propagate entropy failures; signed transfer/update clock failures no longer substitute timestamp zero; DNS UPDATE responses require QR=1, opcode UPDATE, and zero reserved bits; signed updates cannot be sent without a response-verification key. `UpdateBuilder::new` is now fallible and `no_std` callers must use `with_id`. Verification: 670 deterministic workspace tests pass (373 core, 276 net, 19 CLI, one trybuild, one doctest), all-feature clippy is warning-free, both no-default-feature WASM checks pass, and all 27 live BIND 9.20 tests pass. |
...
- **2026-06-20, FR-062 fixtures + extended fuzz evidence**: added a deterministic 24-fixture master-file suite (`zone_roundtrip_fixtures.rs`) satisfying the FR-062 "20+ zone fixtures parse→serialize" criterion — every fixture covers one or more `RecordData` variants and is asserted to round-trip idempotently (serializer is a fixed point and re-parsed records are stable). The live "load into BIND9 → query" half remains covered by the ignored integration suite. Separately, ran an extended fork-mode `cargo-fuzz` campaign across all eight targets (~110s each, all CPU cores) with **zero crash/oom/timeout artifacts**; this is evidence beyond the bounded CI smoke but is **not** the FR-061 24-hour wall-clock gate, which remains an explicit pre-v1.0 release gate. Evidence: 722 deterministic tests pass.
- **2026-06-20, real browser-runtime test (CX-19b)**: added a Playwright headless-Chromium harness (`tests/browser-smoke.mjs` + `tests/browser/index.html`) that serves the bindings dir with COOP/COEP headers (required for SharedArrayBuffer WASM threads), loads the `wasm32-wasip1-threads` browser bundle, and asserts the core API works in-browser. Wired into the `bindings-wasm` CI job (installs Chromium, runs for real). The harness SKIPs cleanly with a non-failing exit when Playwright/Chromium is unavailable (e.g. the local sandbox where `npm install` is blocked), so it never reports a false pass. Browser compatibility is therefore verified in CI rather than asserted from documentation.
- **2026-06-20, napi/WASM packaging verification (CX-19b)**: CI now builds the napi addon and runs the native smoke + package-contract tests on a Node 22/24 matrix, a Bun smoke, and a `wasm32-wasip1-threads` build + WASI fallback smoke. All paths were verified locally against fresh builds (native and WASI). Fixed a stale binding smoke-test expectation (`labelCount` of `example.com.` is 2 — root excluded — not 3); the failure had been masked by a stale committed `.node` artifact that still exported the removed `JsRndcPool`. Build artifacts (`.node`/`.wasm`) remain gitignored and are produced by CI/release, eliminating drift. Not claimed: browser-runtime compatibility (no real browser test was run). Node 20 is excluded by `package.json` `engines` (`>=22`).
...
- **2026-06-20, typed operational security model (CX-15, partial)**: added a no_std `security` module to `bind9-sdk-core` with a typed `SecurityWarning` enum (deprecated TSIG algorithm, weak DNSSEC algorithm, near-expiry/expired RRSIG, cleartext transfer), a `Severity` ordering, a configurable `SecurityPolicy` (`Warn` / `DenyOnWarning` / `DenyOnCritical`) supporting REQ-SEC-DEFAULT-3 fail-closed behavior, and classifiers `tsig_algorithm_is_deprecated`, `dnssec_algorithm_is_weak` (RFC 9904 weak set: 1/3/5/6/7/12), and `classify_rrsig` for RRSIG-expiry monitoring (REQ-LOG-5). Warnings carry no key material. The net `DnssecStatus::weak_algorithm_keys()` reports KASP keys signing with a deprecated algorithm, and `Bind9Client::send_update_verified` performs optional post-update SOA serial verification (REQ-ZONE-3), returning `NetError::UpdateNotApplied` when the serial does not advance. CX-15 is fully implemented. Evidence: 711 deterministic tests pass; all-feature and production-panic clippy and the no-default-feature WASM check pass.
- **2026-06-20, TLS configuration resolved (CX-16)**: removed the false "TLS transports are not implemented yet" warning from `Bind9Client::new`. `ClientConfig::tls` is now consumed — `StatsHttpClient::with_tls` applies caller-supplied trust anchors (TLS 1.3, AES-256-GCM / ChaCha20-Poly1305) to the HTTPS statistics channel, and `Bind9Client::server_stats`/`zone_stats` route through it. Documentation now states accurately that the `TlsConfig` governs HTTPS statistics and certificate-validated XoT zone transfers, and that rndc itself is HMAC-authenticated plaintext (BIND has no rndc-over-TLS); rndc confidentiality is provided by `RndcTransportPolicy::ProtectedNetwork`. Evidence: 700 deterministic tests pass; all-feature and production-panic clippy pass.

- **2026-06-20, supply-chain gate and release engineering (CX-17)**: added `deny.toml` enforcing advisories, bans, licenses, and source policy, wired into CI (`cargo-deny`) and `mise run deny`; `cargo deny check` passes clean. Added a `1.94.0`/`stable` MSRV check matrix, a hardened `[profile.release]` (thin LTO, single codegen unit, symbol strip, overflow checks) plus a `[profile.fuzz]` profile, and a `release.yml` workflow that gates on supply-chain policy, runs a dependency-ordered `cargo package` publish dry run, generates a CycloneDX SBOM with SHA-256 checksums, and emits a build-provenance attestation. Fixed a crates.io publishability gap: the CLI's path dependency on `bind9-sdk` now carries a version. Final license selection (OQ-005) remains an owner decision.
- **2026-06-20, fuzzing and zone-parser DoS fix (FR-061 / CX-05)**: added a detached `fuzz/` cargo-fuzz workspace with eight targets — `fuzz_zone_parser`, `fuzz_rndc_response`, `fuzz_stats_json` (PRD-required) plus `fuzz_named_conf`, `fuzz_zone_roundtrip`, `fuzz_tsig_wire`, `fuzz_transfer_wire`, and `fuzz_update_response` — driven by feature-gated `fuzz` entry modules in `bind9-sdk-core` and `bind9-sdk-net`. Seed corpora come from the live BIND 9.20 fixtures; `--features fuzzing` regression tests pin adversarial inputs deterministically; CI runs a bounded smoke fuzz. Fuzzing-class testing found a genuine stack-overflow DoS — the zone tokenizer recursed once per parenthesized newline and per comment line — now rewritten as an iterative scan with a regression test. Added SR-005 input-size (256 MiB) and record-count (16,777,216) guards enforced before allocation. The PRD FR-061 24-hour clean run is an explicit pre-v1.0 release gate and has **not** been performed. Evidence: 696 deterministic tests pass; all-feature and production-panic clippy, both no-default-feature WASM checks, and `cargo deny check` pass.
- **2026-06-20, mandatory live protocol CI**: GitHub Actions now builds and health-checks the BIND 9.20 fixture, runs all 28 live tests, and tears it down unconditionally. All-feature clippy and explicit production panic-path lints are mandatory both in CI and `mise run clippy`.
- **2026-06-20, bounded named.conf parsing**: implemented FR-060 in `bind9-sdk-core` without introducing `std`. The parser extracts typed zones, TSIG keys, named and inline ACLs, relevant options, RNDC controls, and statistics listeners; it preserves path/line diagnostics for supported syntax and structurally skips out-of-scope directives. The repository's live BIND 9.20 configuration is a regression fixture. Evidence: 689 deterministic tests, all-feature clippy, production panic linting, and no-default-feature WASM checks pass.
- **2026-06-20, transfer termination, wire labels, protected RNDC, panic elimination, and freeze safety**: AXFR and IXFR now reject trailing answer records after a valid completion marker instead of silently ignoring them. DNS wire labels containing dots, backslashes, NUL, or non-UTF-8 octets are converted to lossless master-file escapes. High-level RNDC management remains loopback-only by default and requires an explicit protected-network policy for trusted overlay addresses; the CLI exposes the same policy through `--protected-rndc` and `server.protected_rndc`. Removed production panic paths from validated labels, zone serial lookup, transfer TTL fallback, static UDP binding, CLI runtime/output rendering, and TSIG HMAC construction. TSIG signing and record construction are fallible and validate the 48-bit timestamp and 16-bit prior-MAC length fields. `FrozenZoneGuard` provides explicit awaited thaw and schedules best-effort thaw when dropped, including panic unwinding; live BIND testing covers the freeze/thaw cycle. Evidence: 684 deterministic tests and all 28 live BIND 9.20 tests pass; production-target panic linting, all-feature clippy, and both no-default-feature WASM checks pass.
- **2026-06-20, combined audit and fail-closed protocol construction**: reconciled the independent Claude review, the earlier Codex audit, the current PRD, and a fresh source audit in `docs/audit-combined-2026-06-20.md`. AXFR now verifies that the closing SOA serial matches the opening SOA; IXFR explicitly rejects RFC 1982's undefined half-space comparison. Transfer query IDs, rndc nonces, and RFC 2136 update IDs no longer panic or silently become predictable when entropy fails. Signed transfer and update verification no longer substitutes Unix timestamp zero after a clock error. DNS UPDATE response parsing now requires a response packet with opcode UPDATE and zero reserved bits, and signed messages require the TSIG key needed to verify the response. Evidence: 670 deterministic tests pass; all-feature clippy and both no-default-feature WASM checks pass; all 27 live BIND 9.20 tests pass.
...
| REQ-ZONE-2 | RAII zone-freeze guard: `FrozenZoneGuard` offers explicit awaited `thaw()` and schedules best-effort thaw from `Drop`, including panic unwinding, on the runtime captured at creation | SOC 2 PI1.4 |
...
| OQ-003 | named.conf parser (FR-060): scope creep risk — how much of the named.conf grammar to support? Define explicit in-scope/out-of-scope boundary before v0.6.0 start. | Sephyi | Before v0.6.0 start | RESOLVED — parse SDK-relevant zone/key/ACL/options/controls/statistics statements; structurally skip unknown directives; no include resolution, view expansion, or full grammar emulation. |
| OQ-004 | MSRV: edition 2024 + language features through 1.94. Acceptable? `rust-version = "1.94"` set. | Sephyi | 2026-04-01 | RESOLVED — 1.94 |
| OQ-005 | License: AGPL-3.0-only OR LicenseRef-Commercial is a placeholder. Open-source SDK (MIT/Apache 2.0) likely better for ecosystem adoption. Decide before first crates.io publish. This is a hard blocker for crates.io publish — `cargo publish` will succeed but license non-disclosure may deter adoption. | Sephyi | Before v0.1.0 publish | DEFERRED — no release planned; resolve before any crates.io publish |
