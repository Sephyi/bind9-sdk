...
- 🧪 **719 tests** — unit, property (proptest), snapshot (insta), compile-fail, fuzz-regression, and integration tests, plus a `cargo-fuzz` workspace (FR-061)
...
- 🛡️ **DNSSEC** — DNSKEY, DS, CDS, CDNSKEY, RRSIG, NSEC/NSEC3 record types. CDS generation with SHA-256/SHA-384. DNSSEC rndc commands (sign, validation, checkds). KSK rollover workflow helpers (`RolloverWorkflow`) and weak-algorithm reporting included.
- 📦 **Supply Chain** — `cargo audit` and `cargo deny check` (advisories/bans/licenses/sources) in CI. `#![forbid(unsafe_code)]` in core and net. CycloneDX SBOM, SHA-256 checksums, and build-provenance attestation generated by the release workflow.
